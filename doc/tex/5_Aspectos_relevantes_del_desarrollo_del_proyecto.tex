...
El analizador léxico es generado mediante la herramienta ANTLR4, la cual permite definir los lexemas del lenguaje de forma independiente de la producciones gramaticales donde se van a utilizar. En este proyecto, el archivo que contiene esta información se encuentra en \texttt{src/grammar/Tlexer.g4}.
...
Siguiendo con el ejemplo anterior, se pueden definir las expresiones del lenguaje usando ANTLR4 de la siguiente forma:
\begin{minted}{ANTLR}
expr
...
\begin{itemize}
    \item \mintinline{c}|char *toString(int x)|
    \item \mintinline{c}|void print(const char *first, ...)|
\end{itemize}

La función antes mencionada, \mintinline{c}|char *toString(int x)|, aprovecha internamente funciones estándar de C como \mintinline{c}|int sprintf(char *str, const char *format, ...)| para devolver un \eng{buffer} de tipo \mintinline{c}|char *| sin tener que reinventar este mecanismo y a su vez abstrayendo al programador del uso de estas funciones y el manejo de memoria. 
