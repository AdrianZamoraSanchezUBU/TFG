...
\begin{itemize}
    \item \texttt{\textbf{<nombre\_entrada>}}: argumento obligatorio, indica el nombre del fichero \kw{.T} de entrada.
    \item \texttt{\textbf{-{}-output / -o <nombre\_archivo>}}: argumento obligatorio que especificar el fichero ejecutable de salida. El nombre por defecto del archivo de salida es \kw{out}.
    ...
    \item \texttt{\textbf{-{}-IR <nombre\_archivo>}}: genera un fichero \kw{.ll} con el LLVM IR generado por el compilador. El nombre por defecto del archivo de IR es \kw{ir}.
...
Para conocer los datos necesarios para ejecutar las acciones descritas por los comandos anteriores, el compilador almacena flags de compilado que permiten modificar el comportamiento de cada fase.

\section{Proceso de compilación}
Puesto que el compilador se ha planificado de forma modular, a continuación se recorrerá el proceso de compilación completo, utilizando del contenido del MVP como ejemplo.

\subsection{Análisis léxico}
El analizador léxico es generado mediante la herramienta ANTLR4, la cual permite definir los lexemas del lenguaje de forma independiente de la producciones gramaticales donde se van a utilizar. En este proyecto, el archivo que contiene esta información se encuentra en el archivo \texttt{src/grammar/TLexer.g4}.

En este punto del desarrollo del compilador, por ejemplo, se pueden definir las palabras clave asociadas a los tipos básicos del lenguaje:
\begin{minted}{ANTLR}
    TYPE_INT      : 'int'    ;
    TYPE_FLOAT    : 'float'  ;
    TYPE_CHAR     : 'char'   ;
...
\subsubsection{Comentarios y otros caracteres}
En esta fase se implementa la gestión de caracteres especiales como espacios en blanco, saltos de línea o tabulaciones. Estos elementos se consideran triviales desde el punto de vista sintáctico y, por tanto, son ignorados por el analizador léxico, permitiendo una mayor flexibilidad en el formato del código fuente sin afectar a su significado.

De forma similar, se han de ignorar los comentarios, estructuras destinadas a la documentación del código que no deben alterar su comportamiento. En el lenguaje desarrollado se implementan únicamente comentarios de línea, los cuales comienzan con una doble barra inclinada (\texttt{//}) y se extienden hasta el final de la línea. Esta decisión simplifica el análisis léxico y resulta suficiente para cubrir las necesidades básicas de documentación del código en el contexto del proyecto.
...
Siguiendo con el ejemplo anterior, se definen las expresiones del lenguaje usando ANTLR4 de la siguiente forma:
\begin{minted}{ANTLR}
expr
...
Para formar el AST se empleará un \eng{visitor pattern} (patrón de visita), objeto que recibe el nombre de \kw{ASTBuilder}, el cual se encarga de visitar cada contexto generado durante el análisis sintáctico, dando por resultado una estructura de nodos enlazados, el AST.

Adicionalmente, si la flag \code{-{}-visualizeAST} o \code{-{}-debug} están activadas, el compilador generará una representación visual del árbol de sintaxis abstracta tras su construcción, para conseguir esto se escriben cada nodo en el formato de la biblioteca \eng{forest} de \LaTeX, permitiendo el análisis de la estructura del programa así como las relaciones de sus componentes, los tipos y operaciones.
...
\begin{itemize}
    ...
    \item La verificación del alcance en accesos a los identificadores.
...
A continuación se muestra un ejemplo sencillo en el que la tabla de símbolos gestiona varios \eng{scopes} anidados:
\begin{minted}{c}
// Scope 0
int a = 2;
int b = 3;
...
Como resultado del análisis semántico, se genera la estructura de tabla de símbolos mostrada en la Figura~\ref{fig:tabla-scopes}:
\begin{center}
\begin{lstlisting}
-------------------------------------------------
| Scope #0 (level 0)                            |
...
-------------------------------------------------
\end{lstlisting}
\captionof{figure}{Ejemplo de tabla de símbolos y alcances generada por el analizador semántico}
...
\begin{itemize}
    \item \mintinline{c}|char *intToString(int x)|
    \item \mintinline{c}|char *floatToString(int x)|
    \item \mintinline{c}|int strLen(string x)|
    \item \mintinline{c}|void print(const char *first, ...)|
\end{itemize}

La función antes mencionada, \mintinline{c}|char *toString(int x)|, aprovecha funciones de C como \mintinline{c}|int sprintf(char *str, const char *format, ...)| para devolver un \eng{buffer} de tipo \mintinline{c}|char *| sin tener que reinventar este mecanismo y a su vez abstrayendo al programador del uso de estas funciones y cuestiones como el manejo de memoria. 

\subsection{Generación de LLVM IR}
...
Durante el proceso de generación de código IR, se vuelve a recorrer el AST con un \eng{visitor pattern}, el cual emplea principalmente el objeto \eng{IRGenerator} para generar las instrucciones a partir de los datos contenidos en los nodos del AST.
...
\begin{table}[ht]
    ...
    \toprule
    \textbf{Tipo AST} & \textbf{Tipo LLVM IR} \\
    \midrule
    \kw{TYPE\_INT}     & \kw{i32} \\
...
El uso de estas combinaciones junto a las comprobaciones de calidad mínimas para compilar sin errores, generan un código con un rendimiento similar al código en C optimizado con \code{-O2} de compiladores como GCC, Rustc o Swiftc.

\subsection{Ejemplo con optimizaciones}
En el siguiente ejemplo se puede apreciar cómo las optimizaciones permiten simplificar significativamente el código generado, reduciendo tanto el número de instrucciones como la complejidad del flujo de control, sin alterar el comportamiento del programa.
...
Puede observarse que la condición \code{2 < 3} se evalúa en tiempo de compilación mediante una optimización de \kw{constant folding}, sustituyendo la condición por un valor constante (\mintinline{c}|true|).
...
Dado que los eventos representan \textbf{unidades de ejecución independientes} que pueden activarse de forma diferida o periódica, su correcta gestión resulta clave para garantizar la estabilidad, el control y la robustez del sistema.

Cada evento se ejecuta en un \textbf{hilo} independiente, separado del hilo principal del programa (la función \texttt{main}). Esta decisión de diseño permite aislar la ejecución de los eventos del flujo principal, de modo que un fallo o comportamiento inesperado en un evento no compromete la ejecución global del programa. De esta forma, el sistema mejora su tolerancia a errores y evita que una excepción o bloqueo en un evento provoque la finalización del proceso principal.

La creación, ejecución y finalización de los eventos se encuentra centralizada en el \eng{runtime}, que actúa como gestor de eventos. Aunque los eventos se ejecutan en hilos independientes, el hilo principal mantiene en todo momento el control sobre su estado, pudiendo conocer qué eventos se encuentran activos, cuáles han finalizado y cuáles están pendientes de ejecución. Este enfoque permite combinar concurrencia con supervisión centralizada, evitando la pérdida de control sobre los hilos lanzados dinámicamente.

Los eventos son \textbf{autogestionados} una vez lanzados, se encargan de ejecutar su lógica asociada, control de sus limitaciones de ejecución y periodicidad. Esto reduce el acoplamiento entre el código del usuario y la infraestructura interna del runtime, simplificando tanto el modelo de programación como el mantenimiento del sistema.
...
Para este propósito se exploró el uso de \textbf{libffi}, una biblioteca que permite realizar llamadas a funciones de forma dinámica, especificando en tiempo de ejecución la firma correcta conforme al \eng{application binary interface} (ABI), tanto en la función a invocar como en el retorno de esta y en los tipos de sus parámetros. Aunque esta aproximación resulta adecuada para el paso de parámetros por valor, introduce una complejidad significativa cuando se trata de pasar punteros.

En esta capa nuevamente se definen tipos equivalente para el paso por libffi:
\begin{table}[ht]
    \centering
    \begin{tabularx}{\linewidth}{ p{0.5\columnwidth} p{0.5\columnwidth} }
    \toprule
    \textbf{Tipo LLVM IR} & \textbf{Tipo Libffi} \\
    \midrule
    \kw{i32}   & \kw{ffi\_type\_sint32}  \\
    \kw{float} & \kw{ffi\_type\_uint8}   \\
    \kw{i8}    & \kw{ffi\_type\_sint8}   \\
    \kw{i8*}   & \kw{ffi\_type\_float}   \\
    \kw{i1}    & \kw{ffi\_type\_pointer} \\
    \kw{void}  & \kw{ffi\_type\_void}    \\
    \kw{ptr}   & \kw{ffi\_type\_pointer} \\
    \bottomrule
    \end{tabularx}
    \caption{\texorpdfstring{Correspondencia entre tipos del AST y tipos de LLVM IR}{Correspondencia entre tipos del AST y tipos de LLVM IR}}
\end{table}
...
Con el objetivo de evaluar el rendimiento del compilador desarrollado, para esta tarea se han creado entre tres implementaciones equivalentes de un mismo algoritmo: una escrita en el lenguaje T diseñado en este proyecto, su equivalente en lenguaje C compilado con optimizaciones \texttt{-O2}, y una versión implementada en \texttt{python3}.

El algoritmo seleccionado consiste en el \textbf{cálculo de números primos}~\cite{algoritmoPrimos} hasta un valor $n$, utilizando exclusivamente \textbf{estructuras de control} básicas (condicionales y bucles) y \textbf{operaciones aritméticas}. No se emplean estructuras de datos complejas ni bibliotecas externas, con el fin de centrar la comparación en el coste del flujo de control y del modelo de ejecución de cada lenguaje.
...
Los resultados obtenidos muestran que el código generado por el compilador desarrollado \textbf{presenta un rendimiento muy cercano al del código C optimizado}, con diferencias aproximadas de 7\% a medida que aumenta $n$. En contraste, tanto el programa en C como el generado en T superan ampliamente a Python, siendo los ejecutables compilados un 20x\textasciitilde30x más rápidos que el interpretado.

Estos resultados se explican por el hecho de que tanto el lenguaje desarrollado como C generan código nativo que se ejecuta directamente sobre la arquitectura subyacente, evitando la sobrecarga propia de los lenguajes interpretados. Asimismo, el uso de una representación intermedia eficiente y la delegación de las optimizaciones de bajo nivel en la infraestructura LLVM permiten obtener un rendimiento competitivo pese a tratarse de un compilador desarrollado en el contexto de un Trabajo de Fin de Grado.
